...
export async function updateTraitsFromMessages(userId: string, doubleId: number) {
  try {
    // Récupérer le double IA
    const aiDouble = await db.select()
      .from(aiDoubles)
      .where(eq(aiDoubles.id, doubleId))
      .limit(1);

    if (!aiDouble || aiDouble.length === 0) {
      console.error('Double IA non trouvé');
      return;
    }
    ...
    if (!currentDiagnostic) {
      console.error('Diagnostic initial non trouvé');
      return;
    }
...
- Retourne UNIQUEMENT le JSON, sans markdown, sans code blocks`;
    ...
    console.log('Traits mis à jour avec succès pour userId:', userId);
  } catch (error) {
